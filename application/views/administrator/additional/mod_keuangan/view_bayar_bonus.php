              ...
              <div class='box box-info'>
                ...
                <div class='box-body'>
                  ...
                  <?php
                    ...
                    $pembelian = $this->db->query("SELECT sum((b.jumlah*b.harga_jual)-b.diskon) as total FROM rb_penjualan a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_penjual='admin' AND a.id_pembeli='".$id_pemilikr."' AND a.proses='1'")->row_array();
                    $penjualan_perusahaan = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_kost=b.id_kost
                                                                JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan!='0' AND id_penjual='".$id_pemilik."' AND c.proses='1'")->row_array();
                    $penjualan = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_kost=b.id_kost
                                                                JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan='0' AND id_penjual='".$id_pemilik."' AND c.proses='1'")->row_array();
                    $modal_perusahaan = $this->db->query("SELECT sum(a.jumlah*b.harga_reseller) as total FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_kost=b.id_kost JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_pembeli='konsumen' AND c.proses='1' AND c.id_penjual='".$id_pemilik."' AND b.id_produk_perusahaan!='0'")->row_array();
                    $modal_pribadi = $this->db->query("SELECT sum(a.jumlah*b.harga_beli) as total FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_kost=b.id_kost JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_pembeli='konsumen' AND c.proses='1' AND c.id_penjual='".$id_pemilik."' AND b.id_produk_perusahaan='0'")->row_array();
                    $set = $this->db->query("SELECT * FROM rb_setting where aktif='Y'")->row_array();
                    $res = $this->db->query("SELECT * FROM pemilik where id_pemilik='$id_pemilik'")->row_array();

                    echo "<table class='table table-striped table-condensed'>
                      <tr><td width='190px'>Nama Toko / Reseller</td>                <td> : <b>$res[nama_pemilik]</b></td></tr>
                    ...
                    $total_jual = 0;
                    $total_bonus = 0;
                    $reseller = $this->db->query("SELECT * FROM pemilik where referral='".$res['username']."'");
                    if ($reseller->num_rows()<=0){
                      ...
                      foreach ($reseller->result_array() as $row) {
                        $pp = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_kost=b.id_kost
                                                                  JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan!='0' AND id_penjual='".$row['id_pemilik']."' AND c.proses='1'")->row_array();
                        $total_jual = $total_jual+$pp['total'];
                        $total_bonus = $total_bonus+($set['referral']/100*$pp['total']);
                        echo "<tr><td width='20px'>$no</td>
                                  <td><b>$row[nama_pemilik]</b></td>
                    ...
                    $pen = $this->db->query("SELECT sum(bonus_referral) as pencairan FROM rb_pencairan_bonus where id_pemilik='$id_pemilik'")->row_array();
                    echo "<tr class='alert alert-danger'>
                          ...
                          echo "<input type='hidden' value='$id_pemilik' name='idk'>
                    ...
                    echo "<div class='alert alert-success'><b>Total Penjualan anda Pada Tahun $tahun :</b> </div>
                        ...
                        <table class='table table-bordered table-striped table-condensed'>
                              ...
                              for ($i=1; $i <=12 ; $i++) {
                                ...
                                $ppb = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_kost=b.id_kost
                                                                JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan!='0' AND id_penjual='".$id_pemilik."' AND c.proses='1' AND substr(c.waktu_transaksi,1,7)='$bulan'")->row_array();
                                echo "<tr bgcolor='#e3e3e3'>
                                        ...
                                        <td>";
                                            ...
                                            foreach ($rew->result_array() as $re) {
                                              $cek_reward = $this->db->query("SELECT * FROM rb_pencairan_reward where id_pemilik='$id_pemilik' AND id_reward='$re[id_reward]' AND reward_date='$bulan'");
                                              ...
                                              if ($cek_reward->num_rows()<=0){
                                                echo " <a title='Bayarkan Reward ini' href='".base_url()."administrator/bayar_reward/$id_pemilik/$re[id_reward]/$bulan' onclick=\"return confirm('Apa anda yakin untuk Reward ini sudah dibayarkan?')\"><span class='fa fa-check-square'></span></a>";
                                              }
                                              echo "<br>";
                                              $nomor++;
                                            }
