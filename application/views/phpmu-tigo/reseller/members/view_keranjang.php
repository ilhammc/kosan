...
?>
      ...
      <?php
        echo "<a class='btn btn-success btn-sm' href='".base_url()."members/produk_reseller/$rows[id_pemilik]'>Lanjut Belanja</a>
...
<div class="col-sm-4 colom4">
  ...
  <tbody>
    <tr class='alert alert-info'><th scope='row' style='width:90px'>Pemilik</th> <td><?php echo $res['nama_pemilik']?></td></tr>
    <tr class='alert alert-info'><th scope='row'>Alamat</th> <td><?php echo $res['alamat_lengkap'].', '.$res['nama_kota'].', '.$res['nama_provinsi']; ?></td></tr>
  </tbody>
  </table>

  <?php $usr = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM penyewa a JOIN rb_kota b ON a.kota_id=b.kota_id
                JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id
                  where a.id_penyewa='".$this->session->id_penyewa."'")->row_array(); ?>
  <table class='table table-condensed'>
  <tbody>
