<p class='sidebar-title text-danger produk-title'>Detail Data kost</p>
  ...
  <tbody>
    ...
    <tr><th scope='row' width='140px'>Nama Pemilik</th> <td><?php echo $rows['nama_pemilik']?></td></tr>
  ...
  <hr>
      ...
      if ($record->num_rows()<=0){
          echo "<center style='margin:40px 0px; font-weight:bold; color:#ab0534'>Belum ada Produk pada Pemilik ini...</center>";
      }else{
        $no = 1;
        ...
        if ($beli['beli']-$jual['jual']<=0){ $stok = '<b style="color:red">Kosan Penuh</b>'; }else{ $stok = "Jumlah ".($beli['beli']-$jual['jual'])." $row[satuan]"; }
                  ...
                  <center>
                    ...
                    if ($beli['beli']-$jual['jual']<=0){
                      ...
                      if($this->session->level=='konsumen'){
                        echo "<a class='btn btn-default btn-block btn-sm' href='".base_url()."produk/detail/$row[produk_seo]'>Pesan Sekarang</a>";
                      }else{
                        echo "<a class='btn btn-default btn-block btn-sm' href='".base_url()."produk/detail/$row[produk_seo]'>Pesan Sekarang</a>";
                      }
                    }
