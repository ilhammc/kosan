<?php
$rows = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM `pemilik` a JOIN rb_kota b ON a.kota_id=b.kota_id
JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id where a.id_pemilik='$record[id_pemilik]'")->row_array();
echo "<div class='col-md-12'>
    <div class='col-md-9' style='padding:0px'>
        ...
        $jual = $this->model_reseller->jual_reseller($record['id_pemilik'],$record['id_kost'])->row_array();
        $beli = $this->model_reseller->beli_reseller($record['id_pemilik'],$record['id_kost'])->row_array();
        $disk = $this->db->query("SELECT * FROM kost_diskon where id_kost='$record[id_kost]'")->row_array();
        ...
        <div class='col-md-9' style='padding:0px'>
            ...
            if ($this->session->level=='konsumen'){
                echo "<form action='".base_url()."members/keranjang/$record[id_pemilik]/$record[id_kost]' method='POST'>";
            }else{
                echo "<form action='".base_url()."produk/keranjang/$record[id_pemilik]/$record[id_kost]' method='POST'>";
            }
            echo "<table class='table table-condensed' style='margin-bottom:0px'>
                ...
                <a target='_BLANK' style='border-radius:15px 0px 0px 15px' class='btn btn-danger btn-sm pull-right' href='https://api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=$record[nama_kost],... Apakah%20Kost%20Ini%20bisa%20Nego?...'></a>
                </td></tr>
                <tr><td style='font-weight:bold; width:90px'>Luas</td> <td>$record[berat] M2</td></tr>
                ...
                if (($beli['beli']-$jual['jual'])>=1){
                    echo "<tr><td style='font-weight:bold'>Tersedia</td> <td class='text-success'>".($beli['beli']-$jual['jual'])." sisa kost</td></tr>";
                }else{
            ...
            <div class='alert alert-warning' style='border-radius:0px'>
              ...
              Kost tidak sesuai pesanan? Ikuti langkah retur Pesmesanan di sini.
            </div>

        <center><button type='submit' class='btn btn-success btn-block btn-lg'>Pesan</a></center>";

        echo "</form><br><a target='_BLANK' class='btn btn-default btn-sm' href='https://api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=$record[nama_kost],... Apakah%20Stok%20Masih%20ada?...'><span class='glyphicon glyphicon-comment'></span>  Apakah Kost Masih ada?</a>
                <a target='_BLANK' class='btn btn-default btn-sm' href='https://api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=$record[nama_kost],... Saya%20Pesan%20Sekarang%20ya!'><span class='glyphicon glyphicon-comment'></span> Saya Pesan Sekarang ya!</a>
                <a target='_BLANK' class='btn btn-default btn-sm' href='https://api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=Assalam,%20Haloo!%20$rows[nama_pemilik],%20Saya%20Mau%20Order%20Produknya...'><span style='color:green' class='glyphicon glyphicon-certificate'></span> Chat dengan Pemilik</a>
        </div>
        </div>
        <div class='col-md-12' style='padding:0px'>
            <div class='panel-body'>
                <ul class='myTabs nav nav-tabs' role='tablist'>
                  <li role='presentation' class='active'><a href='#deskripsi' id='deskripsi-tab' role='tab' data-toggle='tab' aria-controls='deskripsi' aria-expanded='true'>Detail Kost</a></li>
                  <li role='presentation' class=''><a href='#diskusi' role='tab' id='diskusi-tab' data-toggle='tab' aria-controls='diskusi' aria-expanded='false'>Komentar</a></li>
                </ul><br>
