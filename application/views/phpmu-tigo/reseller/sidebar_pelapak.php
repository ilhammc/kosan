...
echo "<div class='widget'>
    <h3>Tentang Pemilik</h3>";
      ...
      <tbody>";
        ...
        $sukses = $this->db->query("SELECT * FROM rb_penjualan where id_penjual='$rows[id_pemilik]' AND status_penjual='reseller' AND proses!='0'");
        $pelanggan = $this->db->query("SELECT * FROM rb_penjualan where id_penjual='$rows[id_pemilik]' AND status_pembeli='konsumen' AND status_penjual='reseller' GROUP BY id_pembeli");
        ...
        <tr><td colspan='2'><b style='font-size:16px'>$rows[nama_pemilik]</b></td></tr>
...
<a class='btn btn-xs btn-block btn-success' href='".base_url()."produk/produk_reseller/$rows[id_pemilik]'>Semua Produk</a>
<a class='btn btn-xs btn-block btn-info' href='".base_url()."produk/detail_reseller/$rows[id_pemilik]'>Lihat Profile</a>
