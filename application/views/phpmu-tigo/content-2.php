		...
		<div class="content-block main right">
			...
			<div class="block">
				<div class="block-title">
					<a href="<?php echo base_url(); ?>produk" class="right">+ kost Lainnya</a>
			...
			<?php
			  $no = 1;
			  $record = $this->db->query("SELECT * FROM kost where id_pemilik!='0' AND id_produk_perusahaan='0' ORDER BY id_kost DESC LIMIT 6");
			    foreach ($record->result_array() as $row){
			    $ex = explode(';', $row['gambar']);
			    if (trim($ex[0])==''){ $foto_produk = 'no-image.png'; }else{ $foto_produk = $ex[0]; }
			    if (strlen($row['nama_kost']) > 43){ $judul = substr($row['nama_kost'],0,43).',..';  }else{ $judul = $row['nama_kost']; }
			    ...
			    echo "<li style='width:180px'>
						...
						<div class='article-content'><center>
							<h4><a href='".base_url()."produk/detail/$row[produk_seo]'>$row[nama_kost]</a></h4>
							<span style='color:red;'>".rupiah($row['harga_konsumen'])."</span> - <i>$stok</i><br>
							</center>
							<span class='meta'>";
								if ($this->session->level=='konsumen'){
				                  echo "<a class='btn btn-success btn-block btn-sm' style='color:#fff' href='".base_url()."members/keranjang/$row[id_pemilik]/$row[id_kost]'>Beli Sekarang</a>";
				                }else{
				                  echo "<a class='btn btn-success btn-block btn-sm' style='color:#fff' href='".base_url()."produk/keranjang/$row[id_pemilik]/$row[id_kost]'>Beli Sekarang</a>";
				                }
							echo "</span>
						</div>
					   </li>";
