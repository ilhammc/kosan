                  ...
                  <div class='panel-body'>
                    ...
                    <div id='myTabContent' class='tab-content'>
                      <div role='tabpanel' class='tab-pane fade active in' id='profile' aria-labelledby='profile-tab'>
                          <div class='col-md-12'>
                              <?php
                              $pembelian = $this->model_reseller->pembelian($this->session->id_pemilik)->row_array();
                              $penjualan_perusahaan = $this->model_reseller->penjualan_perusahaan($this->session->id_pemilik)->row_array();
                              $penjualan = $this->model_reseller->penjualan($this->session->id_pemilik)->row_array();
                              $modal_perusahaan = $this->model_reseller->modal_perusahaan($this->session->id_pemilik)->row_array();
                              $modal_pribadi = $this->model_reseller->modal_pribadi($this->session->id_pemilik)->row_array();
                              ...
                              $total_jual = 0;
                              $total_bonus = 0;
                              $reseller = $this->db->query("SELECT * FROM pemilik where referral='".$this->session->username."'");
                              ...
                              $pen = $this->db->query("SELECT sum(bonus_referral) as pencairan FROM rb_pencairan_bonus where id_pemilik='".$this->session->id_pemilik."'")->row_array();
                                  ...
                                  <table class='table table-bordered table-striped table-condensed'>
                                        ...
                                        for ($i=1; $i <=12 ; $i++) {
                                          ...
                                          $ppb = $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN kost b ON a.id_kost=b.id_kost
                                                                          JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan!='0' AND id_penjual='".$this->session->id_pemilik."' AND c.proses='1' AND substr(c.waktu_transaksi,1,7)='$bulan'")->row_array();
                                          echo "<tr bgcolor='#e3e3e3'>
                                                  ...
                                                  <td>";
                                                      ...
                                                      foreach ($rew->result_array() as $re) {
                                                        $cek_reward = $this->db->query("SELECT * FROM rb_pencairan_reward where id_pemilik='".$this->session->id_pemilik."' AND id_reward='$re[id_reward]' AND reward_date='$bulan'");
