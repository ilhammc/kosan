                    ...
                    <tr><th scope='row'>Nama Konsumen</th>                 <td><select class='combobox form-control' name='b'>
                                                                                ...
                                                                                <?php
                                                                                  foreach ($konsumen as $r){
                                                                                    if ($r['id_penyewa']==$rows['id_penyewa']){
                                                                                      echo "<option value='$r[id_penyewa]' selected>$r[nama_lengkap]</option>";
                                                                                    }else{
                                                                                      echo "<option value='$r[id_penyewa]'>$r[nama_lengkap]</option>";
                                                                                    }
                                                                                  }
                                                                                ?>
                                                                               </select></td></tr>
                    ...
                    <?php
                        echo "<tr>
                                ...
                                <td><select name='aa' class='combobox form-control' onchange=\"changeValue(this.value)\" autofocus>
                                                                      ...
                                                                      foreach ($barang as $r){
                                                                        $disk = $this->model_app->edit('kost_diskon',array('id_kost'=>$r['id_kost'],'id_pemilik'=>$this->session->id_pemilik))->row_array();
                                                                        if ($r['id_kost']==$row['id_kost']){
                                                                          echo "<option value='$r[id_kost]' selected>$r[nama_kost]</option>";
                                                                          $jsArray .= "prdName['" . $r['id_kost'] . "'] = {name:'" . addslashes($r['harga_konsumen']-$disk['diskon']) . "',desc:'".addslashes($r['satuan'])."'};\n";
                                                                        }else{
                                                                          echo "<option value='$r[id_kost]'>$r[nama_kost]</option>";
                                                                          $jsArray .= "prdName['" . $r['id_kost'] . "'] = {name:'" . addslashes($r['harga_konsumen']-$disk['diskon']) . "',desc:'".addslashes($r['satuan'])."'};\n";
                                                                        }
                                                                      }
                                                                   echo "</select></td>
