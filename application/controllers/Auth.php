...
class Auth extends CI_Controller {
	...
	public function register(){
		if (isset($_POST['submit1'])){
			...
			$this->session->set_userdata(array('id_penyewa'=>$id, 'level'=>'konsumen'));
	...
	public function login(){
		if (isset($_POST['login'])){
				...
				$cek = $this->db->query("SELECT * FROM penyewa where username='".$this->db->escape_str($username)."' AND password='".$this->db->escape_str($password)."'");
			    $row = $cek->row_array();
			    $total = $cek->num_rows();
				if ($total > 0){
					$this->session->set_userdata(array('id_penyewa'=>$row['id_penyewa'], 'level'=>'konsumen'));
					if ($this->session->idp!=''){
						$data = array('kode_transaksi'=>$this->session->idp,
			        			  'id_pembeli'=>$row['id_penyewa'],
			        			  'id_penjual'=>$this->session->reseller,
			        			  'status_pembeli'=>'konsumen',
			        			  'status_penjual'=>'reseller',
			        			  'waktu_transaksi'=>date('Y-m-d H:i:s'),
	...
	public function lupass(){
		if (isset($_POST['lupa'])){
			...
			$cek = $this->db->query("SELECT * FROM penyewa where email='".$this->db->escape_str($email)."'");
		    $row = $cek->row_array();
		    $total = $cek->num_rows();
			if ($total > 0){
				...
				$this->db->query("UPDATE penyewa SET password='$passwordbaru' where email='".$this->db->escape_str($email)."'");
