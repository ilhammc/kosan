...
class Members extends CI_Controller {
	...
	function profile(){
		...
		$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_penyewa)->row_array();
	...
	function edit_profile(){
		...
		if (isset($_POST['submit'])){
			$this->model_reseller->profile_update($this->session->id_penyewa);
			redirect('members/profile');
		}else{
			$data['title'] = 'Edit Profile Anda';
			$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_penyewa)->row_array();
			$row = $this->model_reseller->profile_konsumen($this->session->id_penyewa)->row_array();
	...
	function reseller(){
		...
		if (is_numeric($dari)) {
			...
			if (isset($_POST['submit'])){
				...
				if ($total->num_rows()==1){
					$row = $total->row_array();
					redirect('produk/keranjang/'.$row['id_pemilik'].'/'.$this->session->produk);
				}
			}else{
	...
	function detail_reseller(){
		...
		$data['record'] = $this->model_reseller->penjualan_list_konsumen($id,'id_pemilik');
	...
	function orders_report(){
		...
		$data['record'] = $this->model_reseller->orders_report($this->session->id_penyewa,'reseller');
		$this->template->load(template().'/template',template().'/reseller/members/view_orders_report',$data);
	}

	function produk_reseller(){
		cek_session_members();
		$jumlah= $this->model_app->view('kost')->num_rows();
		...
		if (is_numeric($dari)) {
			...
			$id = $this->uri->segment(3);
			$data['rows'] = $this->db->query("SELECT * FROM pemilik a JOIN rb_kota b ON a.kota_id=b.kota_id where a.id_pemilik='$id'")->row_array();
			$data['record'] = $this->model_app->view_where_ordering_limit('kost',array('id_pemilik!='=>'0'),'id_kost','DESC',$dari,$config['per_page']);
			$this->pagination->initialize($config);
	...
	function keranjang(){
		...
		if ($id_kost!=''){
			if ($stok <= '0'){
				...
				if ($this->session->idp == ''){
					...
					$data = array('kode_transaksi'=>$kode_transaksi,
				        		  'id_pembeli'=>$this->session->id_penyewa,
				        		  'id_penjual'=>$id_pemilik,
				...
				$cek = $this->model_app->view_where('rb_penjualan_detail',array('id_penjualan'=>$this->session->idp,'id_kost'=>$id_kost))->num_rows();
				if ($reseller['id_penjual']==$id_pemilik){
					if ($cek >=1){
						$this->db->query("UPDATE rb_penjualan_detail SET jumlah=jumlah+$qty where id_penjualan='".$this->session->idp."' AND id_kost='$id_kost'");
					}else{
					...
					if ($this->session->idp != ''){
						...
						$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$this->session->idp),'id_penjualan_detail','ASC');
					}
			...
			if ($this->session->idp != ''){
				$data['rows'] = $this->model_reseller->penjualan_konsumen_detail($this->session->idp)->row_array();
				$data['rowsk'] = $this->model_reseller->view_join_where_one('penyewa','rb_kota','kota_id',array('id_penyewa'=>$this->session->id_penyewa))->row_array();
				$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$this->session->idp),'id_penjualan_detail','ASC');
			}
				$data['title'] = 'Keranjang Belanja';
				$this->template->load(template().'/template',template().'/reseller/members/view_keranjang',$data);
	...
	function keranjang_detail(){
		...
		$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$this->uri->segment(3)),'id_penjualan_detail','ASC');
	...
	function selesai_belanja(){
		if (isset($_POST['submit'])){
			...
			$kons = $this->model_reseller->profile_konsumen($this->session->id_penyewa)->row_array();

			$res = $this->model_app->view_where('pemilik',array('id_pemilik'=>$cekres['id_penjual']))->row_array();
			$data['rekening_reseller'] = $this->model_app->view_where('rb_rekening_reseller',array('id_pemilik'=>$cekres['id_penjual']));
			...
			$message      = "<html><body>Halooo! <b>".$kons['nama_lengkap']."</b> ... <br> Hari ini pada tanggal <span style='color:red'>$tglaktif</span> Anda telah order produk di $iden[nama_website].
				...
				<table style='width:100%;' class='table table-striped'>
			          ...
			          $no = 1;
			          $belanjaan = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$this->session->idp),'id_penjualan_detail','ASC');
			...
			$message .= "<tr bgcolor='#e3e3e3'><td>$no</td>
			          ...
			          $total = $this->db->query("SELECT sum((a.harga_jual*a.jumlah)-a.diskon) as total, sum(b.berat*a.jumlah) as total_berat FROM `rb_penjualan_detail` a JOIN kost b ON a.id_kost=b.id_kost where a.id_penjualan='".$this->session->idp."'")->row_array();
			$message .= "<tr bgcolor='lightgreen'>
			      ...
			      <table style='width:100%;' class='table table-hover table-condensed'>
					...
					<tbody>";
					    $noo = 1;
					    $rekening = $this->model_app->view_where('rb_rekening_reseller',array('id_pemilik'=>$cekres['id_penjual']));
	...
	function order(){
		...
		$cek = $this->db->query("SELECT b.nama_kota FROM penyewa a JOIN rb_kota b ON a.kota_id=b.kota_id where a.id_penyewa='".$this->session->id_penyewa."'")->row_array();
	...
	public function username_check(){
        ...
        if($this->input->is_ajax_request()) {
            ...
            if(!$this->form_validation->is_unique($username, 'penyewa.username')) {
    ...
    public function email_check(){
        ...
        if($this->input->is_ajax_request()) {
	        ...
	        if(!$this->form_validation->is_unique($email, 'penyewa.email')) {
