...
class Reseller extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			$cek = $this->db->query("SELECT * FROM pemilik where username='".$this->db->escape_str($username)."' AND password='".$this->db->escape_str($password)."'");
		    $row = $cek->row_array();
		    $total = $cek->num_rows();
			if ($total > 0){
				$this->session->set_userdata(array('id_pemilik'=>$row['id_pemilik'],
	...
	function ref(){
		if (isset($_POST['submit2'])){
			$cek  = $this->model_app->view_where('pemilik',array('username'=>$this->input->post('a')))->num_rows();
			if ($cek >= 1){
				$username = $this->input->post('a');
				...
				$data = array('username'=>$this->input->post('a'),
		        			  'password'=>hash("sha512", md5($this->input->post('b'))),
		        			  'nama_pemilik'=>$this->input->post('c'),
				...
				$this->model_app->insert('pemilik',$data);
				$id = $this->db->insert_id();
				$this->session->set_userdata(array('id_pemilik'=>$id, 'level'=>'reseller'));
				$identitas = $this->model_app->view_where('identitas',array('id_identitas'=>'1'))->row_array();

				$ref = $this->model_app->view_where('pemilik',array('username'=>$this->input->post('i')))->row_array();
				$email_tujuan = $ref['email'];
				$tglaktif = date("d-m-Y H:i:s");
				$subject      = 'Pendaftaran Sebagai Reseller Berhasil...';

				$message      = "<html><body>Selamat, Pada Hari ini tanggal $tglaktif<br> Bpk/Ibk <b>".$this->input->post('c')."</b> Sukses Mendafatar Sebagai Pemilik dengan referral <b>".$ref['nama_pemilik']."</b>...";
				$message      .= "<table style='width:100%; margin-left:25px'>
						...
						<tr><td><b>Nama Pemilik</b></td>			<td> : ".$this->input->post('c')."</td></tr>
		...
		if (isset($_POST['submit'])){
            ...
            if ($hasil['file_name']==''){
				if (trim($this->input->post('b')) != ''){
		           ...
		           $data = array('nama_pemilik'=>$this->input->post('c'),
		                        'jenis_kelamin'=>$this->input->post('d'),
		                        'alamat_lengkap'=>$this->input->post('e'),
		                        'no_telpon'=>$this->input->post('f'),
		                        'email'=>$this->input->post('g'),
			...
			$this->model_app->update('pemilik', $data, $where);
			redirect($this->uri->segment(1).'/detail_reseller');
		}else{
			$edit = $this->model_app->edit('pemilik',array('id_pemilik'=>$id))->row_array();
	...
	function produk(){
		cek_session_reseller();
		if (isset($_POST['submit'])){
			$jml = $this->model_app->view('kost')->num_rows();
			for ($i=1; $i<=$jml; $i++){
				$a  = $_POST['a'][$i];
				$b  = $_POST['b'][$i];
				$cek = $this->model_app->edit('kost_diskon',array('id_kost'=>$a,'id_pemilik'=>$this->session->id_pemilik))->num_rows();
				if ($cek >= 1){
					if ($b > 0){
						$data = array('diskon'=>$b);
						$where = array('id_kost' => $a,'id_pemilik' => $this->session->id_pemilik);
						$this->model_app->update('kost_diskon', $data, $where);
					}else{
						$this->model_app->delete('kost_diskon',array('id_kost'=>$a,'id_pemilik'=>$this->session->id_pemilik));
					}
				}else{
					if ($b > 0){
						$data = array('id_kost'=>$a,
			                          'id_pemilik'=>$this->session->id_pemilik,
			                          'diskon'=>$b);
						$this->model_app->insert('kost_diskon',$data);
					}
				}
			}
			redirect($this->uri->segment(1).'/produk');
		}else{
			$data['record'] = $this->model_app->view_where_ordering('kost',array('id_pemilik'=>$this->session->id_pemilik),'id_kost','DESC');
	...
	function tambah_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            $this->model_app->insert('kost',$data);
            $id_kost = $this->db->insert_id();
            if ($this->input->post('diskon') > 0){
            	$cek = $this->db->query("SELECT * FROM kost_diskon where id_kost='".$id_kost."' AND id_pemilik='".$this->session->id_pemilik."'");
				if ($cek->num_rows()>=1){
					$data = array('diskon'=>$this->input->post('diskon'));
					$where = array('id_kost' => $id_kost,'id_pemilik' => $this->session->id_pemilik);
					$this->model_app->update('kost_diskon', $data, $where);
				}else{
					$data = array('id_kost'=>$id_kost,
			                      'id_pemilik'=>$this->session->id_pemilik,
			...
			if ($this->input->post('stok') != ''){
				...
				$data = array('kode_transaksi'=>$kode_transaksi,
			        		  'id_pembeli'=>$this->session->id_pemilik,
			        		  'id_penjual'=>'1',
    ...
    function edit_produk(){
        ...
        if (isset($_POST['submit'])){
            ...
            $where = array('id_kost' => $this->input->post('id'),'id_pemilik'=>$this->session->id_pemilik);
            $this->model_app->update('kost', $data, $where);

            if ($this->input->post('diskon') >= 0){
            	$cek = $this->db->query("SELECT * FROM kost_diskon where id_kost='".$this->input->post('id')."' AND id_pemilik='".$this->session->id_pemilik."'");
				if ($cek->num_rows()>=1){
					$data = array('diskon'=>$this->input->post('diskon'));
					$where = array('id_kost' => $this->input->post('id'),'id_pemilik' => $this->session->id_pemilik);
					$this->model_app->update('kost_diskon', $data, $where);
				}else{
					$data = array('id_kost'=>$this->input->post('id'),
			                      'id_pemilik'=>$this->session->id_pemilik,
			                      'diskon'=>$this->input->post('diskon'));
					$this->model_app->insert('kost_diskon',$data);
				}
            ...
            $data['record'] = $this->model_app->view_ordering('rb_kategori_produk','id_kategori_produk','DESC');
            $data['rows'] = $this->model_app->edit('kost',array('id_kost'=>$id,'id_pemilik'=>$this->session->id_pemilik))->row_array();
    ...
    function delete_produk(){
        cek_session_reseller();
        $id = array('id_kost' => $this->uri->segment(3));
        $this->model_app->delete('kost',$id);
        redirect('reseller/produk');
    }
	...
	function rekening(){
		cek_session_reseller();
		$data['record'] = $this->model_app->view_where('rb_rekening_reseller',array('id_pemilik'=>$this->session->id_pemilik));
	...
	function tambah_rekening(){
		cek_session_reseller();
		if (isset($_POST['submit'])){
			$data = array('id_pemilik'=>$this->session->id_pemilik,
	...
	function edit_rekening(){
		...
		if (isset($_POST['submit'])){
			$data = array('id_pemilik'=>$this->session->id_pemilik,
			              ...
			              'no_rekening'=>$this->input->post('b'),
			              'pemilik_rekening'=>$this->input->post('c'));
			$where = array('id_rekening_reseller' => $this->input->post('id'),'id_pemilik' => $this->session->id_pemilik);
	...
	function pembelian(){
		...
		$data['record'] = $this->model_reseller->reseller_pembelian($this->session->id_pemilik,'admin');
	...
	function tambah_pembelian(){
		...
		if(isset($_POST['submit'])){
			...
			$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$this->session->idp),'id_penjualan_detail','DESC');
			$data['barang'] = $this->model_app->view_where_ordering('kost',array('id_pemilik'=>'0'),'id_kost','ASC');
			$data['reseller'] = $this->model_app->view_ordering('pemilik','id_pemilik','ASC');
	...
	function keterangan(){
		cek_session_reseller();
		if (isset($_POST['submit'])){
			$cek = $this->model_app->view_where('rb_keterangan',array('id_pemilik'=>$this->session->id_pemilik))->num_rows();
			if ($cek>=1){
				$data1 = array('keterangan'=>$this->input->post('a'));
				$where = array('id_keterangan' => $this->input->post('id'),'id_pemilik'=>$this->session->id_pemilik);
			...
			$data['record'] = $this->model_app->edit('rb_keterangan',array('id_pemilik'=>$this->session->id_pemilik))->row_array();
		...
		if (isset($_POST['submit1'])){
			if ($this->session->idp == ''){
				$data = array('kode_transaksi'=>$this->input->post('a'),
			        		  ...
			        		  'id_penjual'=>$this->session->id_pemilik,
			...
			$jual = $this->model_reseller->jual_reseller($this->session->id_pemilik, $this->input->post('aa'))->row_array();
            $beli = $this->model_reseller->beli_reseller($this->session->id_pemilik, $this->input->post('aa'))->row_array();
			...
			$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$this->session->idp),'id_penjualan_detail','DESC');
			$data['barang'] = $this->model_app->view_ordering('kost','id_kost','ASC');
			$data['konsumen'] = $this->model_app->view_ordering('penyewa','id_penyewa','ASC');
			if ($this->uri->segment(3)!=''){
				$data['row'] = $this->model_app->view_where('rb_penjualan_detail',array('id_penjualan_detail'=>$this->uri->segment(3)))->row_array();
			}
	...
	function edit_penjualan(){
		...
		if (isset($_POST['submit1'])){
			...
			$cekk = $this->db->query("SELECT * FROM rb_penjualan_detail where id_penjualan='".$this->input->post('idp')."' AND id_kost='".$this->input->post('aa')."'")->row_array();
			$jual = $this->model_reseller->jual_reseller($this->session->id_pemilik, $this->input->post('aa'))->row_array();
            $beli = $this->model_reseller->beli_reseller($this->session->id_pemilik, $this->input->post('aa'))->row_array();
	...
	function detail_konsumen(){
		cek_session_reseller();
		$id = $this->uri->segment(3);
		$edit = $this->model_app->edit('penyewa',array('id_penyewa'=>$id))->row_array();
		$data = array('rows' => $edit);
	...
	function alamat_cod(){
		cek_session_reseller();
		$data['record'] = $this->model_app->view_where('rb_reseller_cod',array('id_pemilik'=>$this->session->id_pemilik));
	...
	function edit_cod(){
		...
		if (isset($_POST['submit'])){
			$data = array('id_pemilik'=>$this->session->id_pemilik,
			              'nama_alamat'=>$this->input->post('a'),
			              'biaya_cod'=>$this->input->post('b'));
			$where = array('id_cod' => $this->input->post('id'),'id_pemilik' => $this->session->id_pemilik);
