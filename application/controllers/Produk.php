...
class Produk extends CI_Controller {
	function index(){
		$jumlah= $this->db->query("SELECT * FROM kost where id_pemilik!='0' AND id_produk_perusahaan='0'")->num_rows();
		...
		if (is_numeric($dari)) {
			if ($this->input->post('kata')){
				...
				$data['record'] = $this->db->query("SELECT a.*, b.nama_pemilik, c.nama_kota FROM kost a LEFT JOIN pemilik b ON a.id_pemilik=b.id_pemilik LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_pemilik!='0' AND a.id_produk_perusahaan='0' AND a.nama_kost LIKE '%".cetak($this->input->post('kata'))."%' ORDER BY a.id_kost DESC LIMIT $dari,$config[per_page]");
			}else{
				$data['title'] = title();
				...
				$data['record'] = $this->db->query("SELECT a.*, b.nama_pemilik, c.nama_kota FROM kost a LEFT JOIN pemilik b ON a.id_pemilik=b.id_pemilik
	                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_pemilik!='0' AND a.id_produk_perusahaan='0' ORDER BY a.id_kost DESC LIMIT $dari,$config[per_page]");
			}
	...
	function kategori(){
		...
		$jumlah= $this->model_app->view_where('kost',array('id_kategori_produk'=>$cek['id_kategori_produk']))->num_rows();
		...
		if (is_numeric($dari)) {
			...
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT a.*, b.nama_pemilik, c.nama_kota FROM kost a LEFT JOIN pemilik b ON a.id_pemilik=b.id_pemilik
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_pemilik!='0' AND a.id_produk_perusahaan='0' AND a.id_kategori_produk='$cek[id_kategori_produk]' ORDER BY a.id_kost DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
	...
	function subkategori(){
		...
		$jumlah= $this->model_app->view_where('kost',array('id_kategori_produk_sub'=>$cek['id_kategori_produk_sub']))->num_rows();
		...
		if (is_numeric($dari)) {
			...
			$this->pagination->initialize($config);
			$data['record'] = $this->db->query("SELECT a.*, b.nama_pemilik, c.nama_kota FROM kost a LEFT JOIN pemilik b ON a.id_pemilik=b.id_pemilik
                                    LEFT JOIN rb_kota c ON b.kota_id=c.kota_id where a.id_pemilik!='0' AND a.id_produk_perusahaan='0' AND a.id_kategori_produk_sub='$cek[id_kategori_produk_sub]' ORDER BY a.id_kost DESC LIMIT $dari,$config[per_page]");
			$this->pagination->initialize($config);
			$this->template->load(template().'/template',template().'/reseller/view_kategori_all',$data);
		}else{
			redirect('main');
	...
	function reseller(){
		$jumlah= $this->model_app->view('pemilik')->num_rows();
		...
		if (is_numeric($dari)) {
			...
			if (isset($_POST['submit'])){
				...
				$data['record'] = $this->db->query("SELECT * FROM pemilik a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id ORDER BY id_pemilik DESC LIMIT $dari,$config[per_page]");
			}
	...
	function detail_reseller(){
		...
		$data['rows'] = $this->model_app->edit('pemilik',array('id_pemilik'=>$id))->row_array();
		$data['record'] = $this->model_reseller->penjualan_list_konsumen($id,'reseller');
		$data['rekening'] = $this->model_app->view_where('rb_rekening_reseller',array('id_pemilik'=>$id));
	...
	function produk_reseller(){
		$id = $this->uri->segment(3);
		$jumlah= $this->model_app->view_where('kost',array('id_pemilik'=>$id))->num_rows();
		...
		if (is_numeric($dari)) {
			$data['title'] = 'Data Pemesanan kost';
			$data['description'] = description();
			$data['keywords'] = keywords();
			$data['rows'] = $this->model_app->edit('pemilik',array('id_pemilik'=>$id))->row_array();
			$data['record'] = $this->model_app->view_where_ordering_limit('kost',array('id_pemilik'=>$id),'id_kost','DESC',$dari,$config['per_page']);
			$this->pagination->initialize($config);
	...
	function keranjang(){
		...
		$j = $this->model_reseller->jual_reseller($id_pemilik,$id_kost)->row_array();
        $b = $this->model_reseller->beli_reseller($id_pemilik,$id_kost)->row_array();
        $stok = $b['beli']-$j['jual'];

        $qty = $this->input->post('qty');
		if ($id_kost!=''){
			if ($stok < $qty){
				$produk = $this->model_app->edit('kost',array('id_kost'=>$id_kost))->row_array();
				$produk_cek = filter($produk['nama_kost']);
				echo "<script>window.alert('Maaf, Stok untuk Produk $produk_cek pada Pelapak ini telah habis, silahkan menunggu atau order dengan pelapak lain!');
                                  window.location=('".base_url()."/produk/produk_reseller/$id_pemilik')</script>";
			}else{
				$this->session->unset_userdata('produk');
				if ($this->session->idp == ''){
					$idp = 'TRX-'.date('YmdHis');
					$this->session->set_userdata(array('idp'=>$idp,'reseller'=>$id_pemilik));
				}

				$cek = $this->model_app->view_where('rb_penjualan_temp',array('session'=>$this->session->idp,'id_kost'=>$id_kost))->num_rows();
				if ($this->session->reseller==$id_pemilik){
					if ($cek >=1){
						$this->db->query("UPDATE rb_penjualan_temp SET jumlah=jumlah+$qty where session='".$this->session->idp."' AND id_kost='$id_kost'");
					}else{
						$harga = $this->model_app->view_where('kost',array('id_kost'=>$id_kost))->row_array();
						$disk = $this->model_app->edit('kost_diskon',array('id_kost'=>$id_kost,'id_pemilik'=>$id_pemilik))->row_array();
					...
					if ($this->session->idp != ''){
						$data['rows'] = $this->model_app->edit('pemilik',array('id_pemilik'=>$this->session->reseller))->row_array();
						$data['record'] = $this->model_app->view_join_where('rb_penjualan_temp','kost','id_kost',array('session'=>$this->session->idp),'id_penjualan_detail','ASC');
					}
	...
	function keranjang_detail(){
		...
		$data['record'] = $this->model_app->view_join_where('rb_penjualan_temp','kost','id_kost',array('session'=>$this->uri->segment(3)),'id_penjualan_detail','ASC');
	...
	function checkouts(){
		if (isset($_POST['submit'])){
				if ($this->session->idp!=''){
					...
					$this->model_app->insert('penyewa',$data);
					...
					$res = $this->model_app->view_where('pemilik',array('id_pemilik'=>$this->session->reseller))->row_array();
					...
					$data['rekening_reseller'] = $this->model_app->view_where('rb_rekening_reseller',array('id_pemilik'=>$this->session->reseller));
					...
					$message      = "<html><body>Halooo! <b>".$this->input->post('a')."</b> ... <br> Hari ini pada tanggal <span style='color:red'>$tglaktif</span> Anda telah order produk di $iden[nama_website].
						...
						<table style='width:100%;'>
							...
							<tr><td width='140px'><b>Nama Reseller</b></td>	<td> : ".$res['nama_pemilik']."</td></tr>
						...
						<table style='width:100%;' class='table table-striped'>
					          ...
					          $no = 1;
					          $belanjaan = $this->model_app->view_join_where('rb_penjualan_detail','kost','id_kost',array('id_penjualan'=>$idp),'id_penjualan_detail','ASC');
					...
					$message .= "<tr bgcolor='#e3e3e3'><td>$no</td>
					          ...
					          $total = $this->db->query("SELECT sum((a.harga_jual*a.jumlah)-a.diskon) as total, sum(b.berat*a.jumlah) as total_berat FROM `rb_penjualan_detail` a JOIN kost b ON a.id_kost=b.id_kost where a.id_penjualan='".$idp."'")->row_array();
					$message .= "<tr bgcolor='lightgreen'>
					      ...
					      <table style='width:100%;' class='table table-hover table-condensed'>
							...
							<tbody>";
							    $noo = 1;
							    $rekening = $this->model_app->view_where('rb_rekening_reseller',array('id_pemilik'=>$this->session->reseller));
	...
	public function detail(){
		...
		$dat = $this->db->query("SELECT * FROM kost where produk_seo='$ids' AND id_pemilik!='0'");
	    $row = $dat->row();
	    $total = $dat->num_rows();
	        if ($total == 0){
	        	redirect('main');
	        }
		$data['title'] = $row->nama_kost;
		$data['record'] = $this->model_app->view_where('kost',array('id_kost'=>$row->id_kost))->row_array();
