	...
	function index(){
		...
		if (isset($_POST['submit'])){
			...
			if (isset($_POST['submit1']) OR $_GET['kode']){
				...
				$row = $this->db->query("SELECT a.id_penjualan, b.id_pemilik FROM `rb_penjualan` a jOIN pemilik b ON a.id_penjual=b.id_pemilik where status_penjual='reseller' AND a.kode_transaksi='$kode_transaksi'")->row_array();
	...
	function tracking(){
		if (isset($_POST['submit1']) OR $this->uri->segment(3)!=''){
			...
			if ($cek->num_rows()>=1){
				...
				$data['description'] = description();
				$data['keywords'] = keywords();
				$data['rows'] = $this->db->query("SELECT * FROM rb_penjualan a JOIN penyewa b ON a.id_pembeli=b.id_penyewa JOIN rb_kota c ON b.kota_id=c.kota_id where a.kode_transaksi='$kode_transaksi'")->row_array();
				$data['record'] = $this->db->query("SELECT a.kode_transaksi, b.*, c.nama_kost, c.satuan, c.berat, c.produk_seo FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan JOIN kost c ON b.id_kost=c.id_kost where a.kode_transaksi='".$kode_transaksi."'");
				$data['total'] = $this->db->query("SELECT a.kode_transaksi, a.kurir, a.service, a.proses, a.ongkir, sum(b.harga_jual*b.jumlah) as total, sum(b.diskon*b.jumlah) as diskon_total, sum(c.berat*b.jumlah) as total_berat FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan JOIN kost c ON b.id_kost=c.id_kost where a.kode_transaksi='".$kode_transaksi."'")->row_array();
